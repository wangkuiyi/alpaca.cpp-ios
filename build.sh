...
if [[ $FORCE_REBUILD == "YES" ]]; then
    echo "┌------------------------------------------------------------------------------┐"
    echo "  Deleting existig $PROJECT frameworks ..."
    echo "└------------------------------------------------------------------------------┘"
    rm -rf "$BUILD_DIR"
fi
...
function build_for_ios() {
    ...
    *)
    ...
    if test -f "$test_file" && lipo -info "$test_file"; then
        echo "Skip building $PROJECT framework for $label."
    else
        echo "┌------------------------------------------------------------------------------┐"
...
function merge_fat_static_library() {
    ...
    src="$BUILD_DIR"/$src_label/$PROJECT.framework/$PROJECT
    dst="$BUILD_DIR"/$dst_label/$PROJECT.framework/$PROJECT
...
xcodebuild -create-xcframework \
    -framework "$BUILD_DIR"/macos-arm64/$PROJECT.framework \
    -framework "$BUILD_DIR"/ios-sim-arm64/$PROJECT.framework \
    -framework "$BUILD_DIR"/ios-dev-arm64/$PROJECT.framework \
    -output "$XCFRAMEWORK"
